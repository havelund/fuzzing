...
def test_freeze_future_past():
    ...
    rule p4: always (
           mk_move_cmd() ->
             [a := distance]
               sofar (
                 mk_move_cmd() implies mk_move_cmd(distance=a)
               )
         )
    """)


def test_cmd_arg_interval_with_freeze():
    run("""
    rule p1: count (5,5) mk_move_cmd()
    rule p2: always (mk_move_cmd() -> [a := distance] (2000 < a < 10000))
    """)
...
def test_time2():
    run("""
    rule p: always [t1 := time] wnext [t2 := time] t1 < t2
    """)
